...
const { default: taxonomyLibrary } = await import(`${LIBS}/scripts/taxonomy.js`);
const { updateLinkWithLangRoot } = await import(`${LIBS}/utils/helpers.js`);
...
export async function loadTaxonomy() {
  ...
  taxonomyModule = await taxonomyLibrary(config, taxonomyRoot);
  if (taxonomyModule) {
    ...
    document.querySelectorAll('[data-topic-link]').forEach((a) => {
      ...
      if (tax) {
        ...
        // eslint-disable-next-line no-console
        window.lana.log(`Trying to get a link for an unknown topic: ${topic} (current page)`, { tags: 'article-feed', severity: 'warning' });
        a.href = '#';
      }
